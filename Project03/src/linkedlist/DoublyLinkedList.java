...
/**
 * DoublyLinkedList: A doubly-linked implementation. An abstract class.
 * Implements the various insert/delete methods on a list of abstract nodes
 * We assume a doubly-linked node.
 * The implementation keeps pointers to the front and rear of the list and
 * maintains its own length.
 * 
 * @author (T.M. Rao)
...
public class DoublyLinkedList<T> implements Enumeration {
	 ...
	 */
	public int length() {
		// return length; This is a O(N) operation
		int len = 0;
	 ...
	 */
	public DoublyLinkedNode<T> elementAt(int index) {
	...
	public void insertAt(DoublyLinkedNode<T> node, int location) {
		...
		// Spacial Case: Insert At end
			...
			DoublyLinkedNode<T> thisNode = elementAt(location);
	 ...
	 */
	public void deleteFirstOccurrence(Object info) {
		DoublyLinkedNode<T> thisNode = firstNode;
		int location = -1;
		while (thisNode != null) {
			location++;
	 ...
	 * occur then nothing is done
	 */
	public void deleteAllOccurrences(Object info) {
		DoublyLinkedNode<T> thisNode = firstNode;
		while (thisNode != null) {
			Object nodeInfo = thisNode.getInfo();
			if (nodeInfo.equals(info)) {
	...
	public boolean equals(DoublyLinkedList<T> other) {
		...
		// Reset current element of lists
		this.resetCurrentElement();
		other.resetCurrentElement();

		// Check if there is a larger element in list
		while (this.hasMoreElements()) {
			if (!this.nextElement().equals(other.nextElement())) {
				return false;
			}
		}
	...
	public static DoublyLinkedList<Integer> merge(DoublyLinkedList<Integer> iList1,
			DoublyLinkedList<Integer> iList2) {
		
		// Make copy of list 1
		DoublyLinkedList<Integer> mergedList = iList1.deepCopy();

		// Reset current element of list
		iList2.resetCurrentElement();

		// Check if there is a larger element in list
		while (iList2.hasMoreElements()) {
			int next = iList2.nextElement();
			mergedList.resetCurrentElement();
			int position = 0;
			while (mergedList.hasMoreElements() && mergedList.nextElement() < next) {
				position++;
			}
			mergedList.insertAt(next, position);
		}
		return mergedList;
	}
